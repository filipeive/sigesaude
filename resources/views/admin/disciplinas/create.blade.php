...
@section('title', $disciplina->exists ? 'Editar Disciplina' : 'Criar Disciplina')

@section('content_header')
    <h1 class="text-primary"><i class="fas fa-pen"></i> {{ $disciplina->exists ? 'Editar Disciplina' : 'Criar Disciplina' }}</h1>
@stop

@section('content')
    <div class="card shadow-lg">
        <div class="card-body">
            <form action="{{ $disciplina->exists ? route('admin.disciplinas.update', $disciplina->id) : route('admin.disciplinas.store') }}" method="POST">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="nivel_id" id="nivel_id" class="form-control" required>
                        @foreach($niveis as $id => $nome)
                            <option value="{{ $id }}" {{ old('nivel_id', $disciplina->nivel_id) == $id ? 'selected' : '' }}>{{ $nome }}</option>
                        @endforeach
                    </select>
                </div>

                <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> {{ $disciplina->exists ? 'Atualizar' : 'Criar' }}</button>

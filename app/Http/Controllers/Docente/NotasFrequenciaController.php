...
class NotasFrequenciaController extends Controller
{
    ...
    /**
     * Exporta a pauta anual da disciplina em formato HTML imprimível.
     * O browser pode guardar o resultado como .html ou usar Ctrl+P / "Guardas como PDF".
     * Quando o projecto integrar maatwebsite/laravel-excel, este método deve ser
     * expandido para devolver um ficheiro .xlsx real.
     */
    public function exportar(Request $request)
    {
        $disciplinaId = $request->get('disciplina_id');
        if (! $disciplinaId) {
            return back()->with('error', 'Disciplina não especificada.');
        }
        ...
        $anoLectivoAtual = AnoLectivo::where('status', 'Ativo')->first();
        if (! $anoLectivoAtual) {
            return back()->with('warning', 'Não há ano lectivo activo.');
        }
        ...
        $ano = $anoLectivoAtual->ano ?? now()->year;

        return response()->view('docente.notas_frequencia.pauta', compact(
            'disciplina', 'estudantes', 'anoLectivoAtual', 'docente', 'ano'
        ))->header('Content-Disposition', 'attachment; filename="pauta-'.Str::slug($disciplina->nome).'-'.$ano.'.html"');
    }
}
